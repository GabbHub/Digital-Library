...
$.widget( "xml.xmlEditor", {
	options: {
		...
		ajaxOptions : {
			...
			xmlRetrievalParams : null,
                        xmlUploadParamimg: null,
                        xmlUploadParamnpg: null
		},
		...
		targetNS: "http://www.tei-c.org/ns/1.0"
	},
	...
	constructEditor: function() {
		...
		if (this.options.documentTitle != null)
			$("<h2/>").html(this.options.documentTitle).appendTo(this.editorHeader);
	...
	uploadXML: function(config) {
		...
		var self = this;
                var data = {xmlString:xmlString,img:this.options.ajaxOptions.xmlUploadParamimg,npg:this.options.ajaxOptions.xmlUploadParamnpg};
		$.ajax({
			url : config.url,
			type : "POST",
			data : data,
			...
			success : function(response) {
				...
				var outcome = config.responseHandler(response);
				 ...
				 if(outcome) {
					...
					$("." + submissionStatusClass).html("Failed").css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
					self.addProblem("Failed to submit xml document",response.alert);
				}
			},
	...
	addProblem: function(message, problem) {
		...
		if (problem !== undefined) {
			if (problem.substring) {
				this.problemsPanel.append(problem.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/è/g,"&egrave;"));
			} else {
				this.problemsPanel.append(problem.message.replace(/</g, "&lt;").replace(/>/g, "&gt;")).replace(/è/g,"&egrave;");
			}
		}
...
DocumentState.prototype.updateStateMessage = function () {
	if (this.isChanged()) {
		$("." + submissionStatusClass).html("Unsaved changes").css("background-color", "#ffffff");
	} else {
		$("." + submissionStatusClass).html("All changes saved").css("background-color", "#ffffff");
	}
};
...
ModifyMenuPanel.prototype.initialize = function (parentContainer) {
	...
	this.menuContainer.css({'max-height': '40vh', 'overflow-y': 'auto'});
	return this;
};
...
ModifyMenuPanel.prototype.setMenuPosition = function(){
	...
	this.menuContainer.css({'max-height': '40vh'});
	return this;
};
